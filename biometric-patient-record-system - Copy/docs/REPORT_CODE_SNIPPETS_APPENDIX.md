...
**Role Based Access Control - app.py**
...
**Protected Patient Photo Route - app.py**
...
**Patient Registration Route - app.py**
...
**Biometric Enrollment During Registration - app.py**
...
**Health Metrics Capture During Registration - app.py**
...
```python
        ...
        if has_metric_data:
            db.execute(
                ...
                (patient_id, systolic_bp, diastolic_bp, heart_rate, spo2,
                 glucose, temperature, respiratory_rate, notes, recorded_by, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    patient_id,
...
**Biometric Encryption - biometric_engine.py**
...
**Biometric Decryption - biometric_engine.py**
...
**Face Image Processing - biometric_engine.py**
...
**Fingerprint Image Processing - biometric_engine.py**
...
**ORB Feature Matching - biometric_engine.py**
...
**Face and Fingerprint Score Calculation - biometric_engine.py**
...
**Biometric Matching Function - biometric_engine.py**
...
**Biometric Patient Retrieval Route - app.py**
...
**AI Risk Engine - ai_risk_engine.py**
...
def predict_risk(patient_row, metric_row, recent_records):
    ...
    if not predicted_diseases:
        ...
        predicted_diseases = [highest[0]] if highest[1] >= 0.45 else ["Stable - Observation Mode"]
...
**AI Risk Assessment Storage - app.py**
...
**Emergency Snapshot Function - app.py**
...
def fetch_emergency_snapshot(db, patient_id):
    ...
    latest_risk = db.execute(
        "SELECT * FROM risk_assessments WHERE patient_id = ? ORDER BY id DESC LIMIT 1",
        (patient_id,),
    ).fetchone()
...
**Family Alert Message Builder - app.py**
...
**Send Family Alert Route - app.py**
...
**Integrity Chain Block Creation - integrity_chain.py**
...
def append_block(db, event_type, payload, actor="system", patient_id=None):
    ...
    db.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (block_index, patient_id, event_type, actor, payload_json, data_hash, previous_hash, block_hash, timestamp),
    )
...
**Integrity Chain Verification - integrity_chain.py**
...
**Emergency Scan User Interface - templates/emergency_scan.html**
