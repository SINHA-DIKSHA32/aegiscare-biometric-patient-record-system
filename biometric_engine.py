...
def match_patient(face_image, fingerprint_image, biometric_rows: Iterable[dict], key_path: Path):
    ...
    for row in biometric_rows:
        stored_image = decrypt_image(Path(row["encrypted_path"]), key_path)
        ...
        if row["biometric_type"] == "face" and face_image is not None:
        ...
        if row["biometric_type"] == "fingerprint" and fingerprint_image is not None:
